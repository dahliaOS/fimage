  ...
  if [[ "$stat_output" =~ Size:\ ([0-9]+) ]]; then
    ...
    if [[ $# -gt 2 && -n "$3" ]]; then
      ...
      if [[ "${newsize}" -le "${size}" ]]; then
        ...
        echo "Image size has to be greater than ${size} bytes.  Recommended value is ${recommended_size} bytes."
        return 1
      fi
    else
    ...
    cp "${fvmraw}" "${fvmimg}"
    echo "${BASH_REMATCH[1]}"
    "default/host_x64/fvm" "${fvmimg}" extend --length "${newsize}" --length-is-lowerbound
    echo "done"
  else
    #fx-error "Could not extend fvm, unable to stat fvm image"
    echo "Could not extend fvm, unable to stat fvm image"
    return 1
  fi
  return 0
}
function fx-zbi {
  "default.zircon/tools/zbi" --compressed="zstd" "$@"
}
